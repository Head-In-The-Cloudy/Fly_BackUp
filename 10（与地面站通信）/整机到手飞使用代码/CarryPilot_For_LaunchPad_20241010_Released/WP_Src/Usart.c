...
void UART2_IRQHandler(void)
{
  ...
  {	
		...
		SDK_Data_Receive_Prepare_GroundStation(ch);
  }
}
...
void ConfigureUART2(void)//与地面站串口 初始化
{
  SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOD);//使能GPIO外设		
  SysCtlPeripheralEnable(SYSCTL_PERIPH_UART2);//使能UART外设
  GPIOPinConfigure(GPIO_PD6_U2RX);//GPIO模式配置 PC6--RX PC7--TX 
  GPIOPinConfigure(GPIO_PD7_U2TX);
  GPIOPinTypeUART(GPIO_PORTD_BASE, GPIO_PIN_6 | GPIO_PIN_7);//GPIO的UART模式配置
  UARTConfigSetExpClk(UART2_BASE, SysCtlClockGet(), 115200,
                      (UART_CONFIG_WLEN_8 | UART_CONFIG_STOP_ONE |
                       UART_CONFIG_PAR_NONE));
  UARTFIFODisable(UART2_BASE);//使能UART0中断	
  UARTIntEnable(UART2_BASE,UART_INT_RX);//使能UART3接收中断		
  UARTIntRegister(UART2_BASE,UART2_IRQHandler);//UART3中断地址注册	
