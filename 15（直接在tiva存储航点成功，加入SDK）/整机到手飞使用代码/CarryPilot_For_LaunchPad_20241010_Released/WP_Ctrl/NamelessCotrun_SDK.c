...
vector2f MAPing_XY={100,100};
vector2f pre_MAPing_XY={100,100};
uint8_t FLY_PORT[100][2]={0};
uint8_t if_FLY[7][9]={0};



uint8_t count_FLY_PORT(void)
{
	static uint8_t count_FLY_PORT=0;
    for(int i = 0;i<200;i++)
	{
		if(FLY_PORT[i][0]==0&&FLY_PORT[i][1]==0)
		{
			return count_FLY_PORT;
		}
		else
		{
			count_FLY_PORT++;
...
{
    static uint8_t Write_In_FLY_PORT_count = 0;

    // 第一次写入，直接存当前点
    if (pre_MAPing_XY.A == 100 && pre_MAPing_XY.B == 100) 
    {
        FLY_PORT[Write_In_FLY_PORT_count][0] = MAPing_XY.A+1;
        FLY_PORT[Write_In_FLY_PORT_count][1] = MAPing_XY.B+1;
    ...
    if (MAPing_XY.A == pre_MAPing_XY.A)
    {
        if (MAPing_XY.B > pre_MAPing_XY.B)   // 向上补
        {
            ...
            {
                FLY_PORT[Write_In_FLY_PORT_count][0] = MAPing_XY.A+1;
                FLY_PORT[Write_In_FLY_PORT_count][1] = b+1;
                Write_In_FLY_PORT_count++;
            }
        }
    ...
    else if (MAPing_XY.B == pre_MAPing_XY.B)
    {
        if (MAPing_XY.A > pre_MAPing_XY.A)   // 向右补
        {
            ...
            {
                FLY_PORT[Write_In_FLY_PORT_count][0] = a+1;
                FLY_PORT[Write_In_FLY_PORT_count][1] = MAPing_XY.B+1;
                Write_In_FLY_PORT_count++;
            }
        }
...
extern uint8_t MAP_flag_case_0xA1;
  ...
  target->sdk_mode=*(data_buf+2);
		...
		switch(target->sdk_mode)
		{
			...
			case 0xA1:  //MAPING
			{
				...
				pre_MAPing_XY.A=MAPing_XY.A;
				pre_MAPing_XY.B=MAPing_XY.B;
				MAPing_XY.A = 7-(target->reserved1);
				MAPing_XY.B = 9-(target->reserved2);
							
				
				if(MAPing_XY.A==0&&MAPing_XY.B==0&&MAP_flag_case_0xA1==0)
				{
					MAP_flag_case_0xA1+=1;
				}
				else if(MAPing_XY.A==0&&MAPing_XY.B==0&&MAP_flag_case_0xA1==1)
				{
					MAP_flag_case_0xA1+=1;
...
void SDK_DT_Send_To_GroundStation_MAP_BUG(void)
{
  ...
  sdk_data_to_send[3]=0x02;    // 发送给 地面站的 是固定的数据
    ...
    sdk_data_to_send[4]=0x00;
	sdk_data_to_send[5]=0x00;
