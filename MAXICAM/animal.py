...
def detect_function(x, y):
    ...
    for label in animal_labels:
        ...
        if len(coords) < most_common:
            avg_x, avg_y = 0, 0  # 未达到数量，默认为0
        else:
            # 使用前 `most_common` 个坐标求平均
    ...
    for label, coord in zip(animal_labels, final_find):
        print(f"{label}: {coord}")

    # 发送目标给飞控
    for i, (fx, fy) in enumerate(final_find):
        if final_counts[animal_labels[i]] == 0:
            continue  # 未识别到该类动物，不发坐标
        target.x = fx
        target.y = fy
        target.state = 0
        while target.state != 1 and ctr.work_mode == 2:
