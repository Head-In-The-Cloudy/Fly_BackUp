...
unsigned char sdk_data_to_send[30];
...
void SDK_DT_Send_Check_Search_LandColor(unsigned char mode,uint8_t color,uint8_t shape,COM_SDK com)
{
  ...
  sdk_data_to_send[1]=0xFC;
  sdk_data_to_send[2]=0xA0;
  sdk_data_to_send[3]=4;
  sdk_data_to_send[4]=mode;
  sdk_data_to_send[5]=color;    //颜色
  sdk_data_to_send[6]=shape;   //形状
  sdk_data_to_send[7]=0;        //保留


  u8 sum = 0;
  for(u8 i=0;i<8;i++) sum += sdk_data_to_send[i];
  sdk_data_to_send[8]=sum;
	if(com==UART3_SDK) USART3_Send(sdk_data_to_send, 9);
	if(com==UART0_SDK) USART0_Send(sdk_data_to_send, 9); 
}
...
void SDK_DT_Send_To_Maxicam_Mode_1(void)  //设置工作模式1  并 发送若干个 禁飞区
{
  sdk_data_to_send[0]=0xFF;
  sdk_data_to_send[1]=0xFC;
  sdk_data_to_send[2]=0xA0;
  sdk_data_to_send[3]=1+2*To_TIVA_Target.Forbidden_Rigion_Num;
  sdk_data_to_send[4]=0x01;
	for(int i=0;i<To_TIVA_Target.Forbidden_Rigion_Num;i++)
	{
		sdk_data_to_send[5+2*i]=(8-To_TIVA_Target.Forbidden_Rigion[i].B);
		sdk_data_to_send[5+2*i+1]=To_TIVA_Target.Forbidden_Rigion[i].A;
	}
	
  u8 sum = 0;
  for(u8 i=0;i<5+2*To_TIVA_Target.Forbidden_Rigion_Num;i++) sum += sdk_data_to_send[i];
  sdk_data_to_send[5+2*To_TIVA_Target.Forbidden_Rigion_Num]=sum;
  USART3_Send(sdk_data_to_send, 5+2*To_TIVA_Target.Forbidden_Rigion_Num+1);
}
...
void SDK_DT_Send_To_Maxicam_Mode_1_REP(void)  //应答
{
  ...
  sdk_data_to_send[1]=0xFC;
  sdk_data_to_send[2]=0xA0;
  sdk_data_to_send[3]=0x02;
  sdk_data_to_send[4]=0x01;
  sdk_data_to_send[5]=0x01;
  u8 sum = 0;
  for(u8 i=0;i<6;i++) sum += sdk_data_to_send[i];
  sdk_data_to_send[6]=sum;
  USART3_Send(sdk_data_to_send, 7);
}
...
vector2f MAPing_XY={0,0};
Animal_Count MAX_to_TIVA_Animal_info={0,0,0,0,0};
FLY_Condition MAX_to_TIVA_FLY_info={0,0,0};
...
void SDK_DT_Reset()
{
  ...
  Opv_Top_View_Target.flag=0;   //MAPING mode 收到 一阵消息
...
extern uint8_t MAX_to_TIVA_State;
uint8_t this_point_have_animal=0;
...
void Openmv_Data_Receive_Anl_1(uint8_t *data_buf,uint8_t num,Target_Check *target)
{
  ...
  if(!(*(data_buf)==0xFF && *(data_buf+1)==0xFC))return;//不满足帧头条件
	...
	target->flag			=*(data_buf+10);    //mode 1  recceive  once 
	target->state		  =*(data_buf+11);		// mode1 end flag
  ...
  target->sdk_mode=*(data_buf+2);
		...
		switch(target->sdk_mode)
		{
			...
			case 0xA1:  //MAPING
			{
				MAPing_XY.A = target->reserved1;
				MAPing_XY.B = target->reserved2;
				
				if(MAPing_XY.A==7&&MAPing_XY.B==9&&MAX_to_TIVA_State==0)
				{
					MAX_to_TIVA_State+=1;
				}
				else if(MAPing_XY.A==7&&MAPing_XY.B==9&&MAX_to_TIVA_State==1)
				{
					MAX_to_TIVA_State+=1;
				}
			...
			case 0xA2:  //MAPING
			{
				MAX_to_TIVA_Animal_info.bird_count=target->reserved4;
				MAX_to_TIVA_Animal_info.elephent_count=target->reserved1_int32;
				MAX_to_TIVA_Animal_info.monkey_count=target->reserved2_int32;
				MAX_to_TIVA_Animal_info.tiger_count=target->reserved3_int32;
				MAX_to_TIVA_Animal_info.wolf_count=target->reserved4_int32;
				
				if(MAX_to_TIVA_Animal_info.bird_count
				  +MAX_to_TIVA_Animal_info.elephent_count
				  +MAX_to_TIVA_Animal_info.monkey_count
				  +MAX_to_TIVA_Animal_info.tiger_count
				  +MAX_to_TIVA_Animal_info.wolf_count==0)
				{
					//没动物
					this_point_have_animal=0;
					MAX_to_TIVA_FLY_info.target_x=0;
					MAX_to_TIVA_FLY_info.target_y=0;
					MAX_to_TIVA_FLY_info.fly_direction=target->reserved3;
					
					
				}
				else
				{  //有动物
					this_point_have_animal=1;
					MAX_to_TIVA_FLY_info.target_x=target->x;
					MAX_to_TIVA_FLY_info.target_y=target->y;
					MAX_to_TIVA_FLY_info.fly_direction=0;
				}
				target->flag=1;
			}	
			break;
...
void Openmv_Data_Receive_Anl_2(uint8_t *data_buf,uint8_t num,Target_Check *target)   //前视  摄像头  的串口  
{
  uint8_t sum = 0;
	if(!(*(data_buf)==0xFF && *(data_buf+1)==0xFC))return;//不满足帧头条件
		...
		switch(target->sdk_mode)
		{
			case 0xA4://AprilTag检测
			{
				...
				if(target->flag!=0)  
				{
					...
					else target->trust_flag=1;    //这里可以查看 april tag id
				}
				else 
				{
					target->trust_cnt/=2;
...
GroundStation_To_F4_Target To_F4_Target;
GroundStation_To_TIVA_Target To_TIVA_Target;
Animal animal_all[3]={0};
...
void SDK_DT_Send_To_GroundStation(GroundStation_To_F4_Target* target)
{
  ...
  sdk_data_to_send[1]=0xFC;
  sdk_data_to_send[2]=0xA0;
  sdk_data_to_send[3]=0x02*target->animal_type_num+2;    // 发送给 地面站的 是固定的数据

  for(int i=0;i<target->animal_type_num;i++)
	{
		sdk_data_to_send[4+2*i]=To_F4_Target.animal_all[i].animal_type; // 动物种类
		sdk_data_to_send[4+2*i+1]=To_F4_Target.animal_all[i].animal_num;  //动物数量
		
	}
	sdk_data_to_send[4+2*target->animal_type_num]=target->Find_Animal_Rigion.A;
	sdk_data_to_send[4+2*target->animal_type_num+1]=target->Find_Animal_Rigion.B;
	
  u8 sum = 0;
  for(u8 i=0;i<4+2*target->animal_type_num+2;i++) sum += sdk_data_to_send[i];
  sdk_data_to_send[4+2*target->animal_type_num+2]=sum;  //校验和
  USART2_Send(sdk_data_to_send, 4+2*target->animal_type_num+3);    //打包好之后发送
...
void SDK_DT_Send_To_GroundStation_MAP(void)
{
  ...
  sdk_data_to_send[2]=0xA0;
  sdk_data_to_send[3]=0x02;    // 发送给 地面站的 是固定的数据
	
	
    sdk_data_to_send[4]=MAPing_XY.A;
	sdk_data_to_send[5]=MAPing_XY.B;

	
  u8 sum = 0;
  for(u8 i=0;i<6;i++) sum += sdk_data_to_send[i];
  sdk_data_to_send[6]=sum;  //校验和
  USART2_Send(sdk_data_to_send, 7);    //打包好之后发送
...
void SDK_Data_Receive_Prepare_GroundStation(uint8_t data)
{
  ...
  else if(state[label]==5)//最后接收数据校验和
  {
    state[label] = 0;
    _buf[label][4+_data_cnt[label]]=data;
		Data_Receive_GroundStation(_buf[label],_data_cnt[label]+5,&To_TIVA_Target);
  }
  else 
	{
		state[label] = 0;
...
//把地面站传过来的 禁区信息 存进 To_TIVA_Target的内容当中   
//在串口数据解析完毕且正确 之后 会置位 一个标志位，Get_Forbidden_Rigion=1
//在主函数当中会 对这个变量To_TIVA_Target->Get_Forbidden_Rigion 进行轮询  当发现变量置位  把这个变量置为0  并进行路线的规划生成
...
void GroundStation_Begin_MAP(void)  //主函数中会 一直调用他
{
    ...
    if(sta_cnt==0)
    {
        if(To_TIVA_Target.Get_Forbidden_Rigion==1)
        {
			...
			To_TIVA_Target.Forbidden_Rigion_Num/=2; //每一组数据是两个，所以之后调用要/2
