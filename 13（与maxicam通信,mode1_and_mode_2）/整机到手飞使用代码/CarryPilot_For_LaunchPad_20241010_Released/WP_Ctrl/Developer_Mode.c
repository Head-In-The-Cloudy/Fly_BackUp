...
void Auto_Flight_Ctrl(uint8_t *mode)
{
	...
	case 25:
	{
		Traverse_Search_Track_Land();
		// Traverse_Search_Track_Land_Test();
		// 用户预留任务，编写后注意加上break跳出
	}
	break;
	case 26:
	{
		Traverse_Search_3D_Track();
		// 用户预留任务，编写后注意加上break跳出
	}
	break;
	case 27:
	{
		Traverse_Search_Animal_Find();
		// 用户预留任务，编写后注意加上break跳出
	}
	break;
	case 28:
