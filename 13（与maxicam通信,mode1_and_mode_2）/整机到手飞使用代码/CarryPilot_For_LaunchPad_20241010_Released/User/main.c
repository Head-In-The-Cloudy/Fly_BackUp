...
@日期：2024/10/01
*************************************************************/
uint8_t MAX_to_TIVA_State = 0;
extern uint8_t this_point_have_animal;
...
int main(void)
{
  ...
  {	
	  ...
	  //SDK_DT_Send_To_Maxicam_Mode_1_REP();
	  GroundStation_Begin_MAP();
	  if(Opv_Top_View_Target.flag==1&&(MAX_to_TIVA_State==0||MAX_to_TIVA_State==1))  //收到 MAP下的一个 路径
	  {
			Opv_Top_View_Target.flag=0;
		  SDK_DT_Send_To_GroundStation_MAP();
			SDK_DT_Send_To_Maxicam_Mode_1_REP();

	  }
	  else if(Opv_Top_View_Target.flag==1&&MAX_to_TIVA_State==2)
	  {
			SDK_DT_Send_To_Maxicam_Mode_1_REP(); //不能一直发，考虑用状态机
			MAX_to_TIVA_State++;
	  ...
	  {}
	    ...
	    // 以下两个 状态的 具体 执行 都要放置  在  sdk 的 状态机 里面 或者 直接在 sdk 状态机里面 去轮询 this——point have ani  标志位
	  if(this_point_have_animal==0)
	  {
			//修改下一个目标航点  为    MAX_to_TIVA_FLY_info.fly_direction  和 全局坐标 结合之后 的NavigationTarget
			this_point_have_animal=2;  //2 是处在 接受状态
	  }
	  else if(this_point_have_animal==1)
	  {
			this_point_have_animal=2;
		  //进入寻色块模式   
	  }
	  else 
	  {
